  ...
  calculateTotalHours() {
    this.users = this.users.map(user => {
      ...
      if (contrat && contrat.work_days) {
        ...
        const contratHours = typeof contrat.contrat_hours === 'string' 
          ? parseFloat(contrat.contrat_hours) 
          : contrat.contrat_hours;
      ...
      return {
        ...user,
        totalHours : contrat?.contrat_hours + 'h' || 'N/A'
  ...
  viewContractDetails(user: User) {
    ...
    if (contrat) {
      this.messageService.add({
        ...
        detail: `Type: ${contrat.contrat_type}, Spécialité: ${contrat.speciality}, Heures/semaine: ${contrat.contrat_hours}`
      });
    } else {
      this.messageService.add({
        severity: 'warn',
