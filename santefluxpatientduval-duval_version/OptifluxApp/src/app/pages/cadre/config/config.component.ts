...
export class ConfigComponent {
  ...
  showEditDialog(service: Service) {
    ...
    this.serviceForm.patchValue({
      name: service.name,
      head: selectedCadre || null
    });
    
    // Afficher un avertissement si l'utilisateur n'existe plus
    if (!selectedCadre && service.head) {
      this.showWarning(`Attention: Le responsable actuel "${service.head}" n'existe plus dans la base de données. Veuillez sélectionner un nouveau responsable.`);
    }
  ...
  showEditDialogPole(pole: Pole) {
    ...
    // Afficher un avertissement si l'utilisateur n'existe plus
    if (!selectedCadre && pole.head) {
      this.showWarning(`Attention: Le responsable actuel "${pole.head}" n'existe plus dans la base de données. Veuillez sélectionner un nouveau responsable.`);
    }
