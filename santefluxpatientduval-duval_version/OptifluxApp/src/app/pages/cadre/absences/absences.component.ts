...
@Component({
  ...
  imports: [TableModule, CommonModule, TabViewModule, ToastModule, BadgeModule, ButtonModule,FormsModule, DropdownModule],
...
export class AbsencesComponent implements OnInit {
  ...
  loadAllReservationBox(): void {
    ...
    /*const allReservations = this.allBoxPlan.filter(
      ...
      box_plan => box_plan.staff_id === this.loggedInUserId && ['Annulé', 'Plus disponible'].includes(box_plan.status)
    );*/

    const allReservations = this.allBoxPlan
    .filter(box_plan => 
      ['Plus disponible', 'Refusé', 'Réservé'].includes(box_plan.status)
    )
    .sort((a, b) => {
  ...
  loadAllReservationEncours(): void {
    ...
    .filter(
      //box_plan => box_plan.staff_id === this.loggedInUserId
      box_plan => ['En cours'].includes(box_plan.status)
    )
    .sort((a, b) => {
      // Tri décroissant par date de création (les plus récentes en premier)
      const dateA = a.created_at ? new Date(a.created_at) : new Date(0);
  ...
  acceptReservation(boxPlanId: string): void {
    const boxPlan = this.allBoxPlan.find(bp => bp.id === boxPlanId);
    if (!boxPlan) {
      this.showError('Réservation introuvable');
      return;
    ...
    this.boxPlanService.updateBoxPlanStatus(
      ...
      // Mettre à jour le statut de la salle après l'annulation
      switchMap(() => {
          return this.roomService.updateRoomStatus(boxPlan.room, 'Réservé');
      })
    ).subscribe({
      next: () => {
        this.loadAllData();
        this.showSuccess('Demande de réservation acceptée');
      },
      error: (err) => {
        console.error('Error accepting request:', err);
        this.showError(err.error?.detail || 'Échec de l\'acceptation');
  ...
  refuseReservation(boxPlanId: string): void {
    const boxPlan = this.allBoxPlan.find(bp => bp.id === boxPlanId);
    if (!boxPlan) {
      this.showError('Réservation introuvable');
      return;
    ...
    this.boxPlanService.updateBoxPlanStatus(
      ...
      next: () => {
        this.loadAllData();
        this.showSuccess('Demande de réservation refusée');
      },
      error: (err) => {
        console.error('Error refusing request:', err);
        this.showError(err.error?.detail || 'Échec du refus');
