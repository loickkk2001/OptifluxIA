...
from crud.service import create_service, delete_service, update_service, generate_service_matricule
    ...
    try:
        ...
        for item in data:
            service_data = {
                "name": item.get("name"),
                "head": item.get("head"),
                "matricule": item.get("matricule", generate_service_matricule),
        ...
        return {"message": f"{len(inserted_ids)} services créés avec succès", "data": inserted_ids}
